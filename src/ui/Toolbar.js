...
const TOOL_NAMES = ['RAKE', 'ROCK', 'SHRUB', 'TEAHOUSE', 'CLEAR', 'SOUND', 'SAVE'];
...
export class Toolbar {
  ...
  create() {
    ...
    const btnW = Math.min(70, Math.floor(W / TOOL_NAMES.length) - 8);
  ...
  setActiveTool(name) {
    ...
    this.buttons.forEach((btn) => {
      if (btn.name === 'SOUND' || btn.name === 'SAVE') return;
