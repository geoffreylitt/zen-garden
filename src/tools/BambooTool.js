...
import { createBambooTexture } from '../graphics/sprites/BambooSprite.js';

export class BambooTool {
  ...
  onDown(pointer) {
    ...
    const key = createBambooTexture(this.scene);
    const sprite = this.scene.add.image(pointer.x, pointer.y, key);
    // Anchor at the base so the stick appears planted at the click point
