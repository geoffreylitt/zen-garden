import { SAND_H, TINY_TINE_COUNT, TINY_TINE_SPACING, GROOVE_COLOR, RIDGE_COLOR } from '../constants.js';

export class TinyRakeTool {
  ...
  rakeStroke(from, to) {
    ...
    const halfWidth = ((TINY_TINE_COUNT - 1) * TINY_TINE_SPACING) / 2;

    for (let s = 0; s <= steps; s++) {
      const cx = from.x + nx * s;
      const cy = from.y + ny * s;

      for (let t = 0; t < TINY_TINE_COUNT; t++) {
        const offset = -halfWidth + t * TINY_TINE_SPACING;
