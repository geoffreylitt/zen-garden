import { SAND_H, MEGA_TINE_COUNT, MEGA_TINE_SPACING, GROOVE_COLOR, RIDGE_COLOR } from '../constants.js';

export class MegaRakeTool {
  ...
  rakeStroke(from, to) {
    ...
    const halfWidth = ((MEGA_TINE_COUNT - 1) * MEGA_TINE_SPACING) / 2;

    for (let s = 0; s <= steps; s++) {
      const cx = from.x + nx * s;
      const cy = from.y + ny * s;

      for (let t = 0; t < MEGA_TINE_COUNT; t++) {
        const offset = -halfWidth + t * MEGA_TINE_SPACING;
